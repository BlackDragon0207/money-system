...
module.exports = {
  ...
  async run (client, message, args) {
    ...
    if(slots != null && slotsTimeout - (Date.now() - slots) > 0) {
        ...
        .setTitle(`${message.author.username}님의 슬롯`)
        .setColor(color)
        .setDescription(`얻은 금액 | ${amount}원`)
        .addField(`슬롯`, emojis)
        message.channel.send(embed)
        db.get(`wallet_${message.author.id}`)
        await db.add(`wallet_${message.author.id}`, amount)
        db.get(`slots_${message.author.id}`) 
        await db.set(`slots_${message.author.id}`, Date.now())
    }
}}