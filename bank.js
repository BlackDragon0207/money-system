...
module.exports = {
  ...
  async run (client, message, args) {
    ...
    if(args[0] === 'all') {
        ...
        .setColor('RANDOM')
        .setDescription(`
        <@${user.user.id}> 은행에 모든 금액인 **${balance}**원을 넣었습니다.
        `)
        message.channel.send(embed)
        db.get(`wallet_${user.id}`)
        await db.subtract(`wallet_${user.id}`, balance)
        db.get(`bank_${message.author.id}`)
        await db.add(`bank_${message.author.id}`, balance) 
    } else {
        const amount = args[0]
        if(!amount) message.reply("은행에 넣을 돈을 설정하셔야 합니다!")
            ...
            .setColor('RANDOM')
            .setDescription(`
            <@${user.user.id}> 은행에 넣은 금액은 **${amount}**원 입니다.
            `)
            message.channel.send(embed)
            db.get(`wallet_${user.id}`)
            await db.subtract(`wallet_${user.id}`, amount)
