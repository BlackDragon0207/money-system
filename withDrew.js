...
module.exports = {
  ...
  async run (client, message, args) {
    ...
    if(bank === null || bank === 0) return message.reply("돈이 없습니다!");
    if(args[0] === 'all') {
        ...
        .setColor('RANDOM')
        .setDescription(`
        <@${user.user.id}> 은행에 모든 금액인 **${bank}**원을 인출 하셨습니다.
        `)
        message.channel.send(embed)
        db.get(`bank_${user.id}`)
        await db.subtract(`bank_${user.id}`, bank)
        db.get(`wallet_${message.author.id}`)
        await db.add(`wallet_${message.author.id}`, bank) 
    } else {
        const amount = args[0]
        if(!amount) message.reply("은행에서 인출할 금액을 설정하셔야 합니다!")
            ...
            .setColor('RANDOM')
            .setDescription(`
            <@${user.user.id}> 은행에서 인출한 금액은 **${amount}**원 입니다.
            `)
            message.channel.send(embed)
            db.get(`bank_${user.id}`)
            await db.subtract(`bank_${user.id}`, amount)
